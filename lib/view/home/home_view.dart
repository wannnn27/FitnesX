...
class _HomeViewState extends State<HomeView> {
    ...
    try {
      ...
      if (user != null) {
        ...
        if (doc.exists) {
          ...
          if (userName.isEmpty && data['name'] != null) userName = data['name'];
                                ...
                                PieChartData(
                                  ...
                                  sections: [
                                    PieChartSectionData(
                                      ...
                                      value: (userBMI != null && userBMI! < 40) ? userBMI! : 20,
                                      title: '',
                                      radius: 55,
                                      badgeWidget: userBMI != null ? Text(userBMI!.toStringAsFixed(1), style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700)) : null,
                                    ),
                                    PieChartSectionData(
                                      color: Colors.white,
                                      value: (userBMI != null && userBMI! < 40) ? (40 - userBMI!) : 20,
                                      title: '',
                                      radius: 45,
                                    ),
                                  ],
                  ...
                  child: Container(
                    ...
                    child: Stack(
                      ...
                      children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              ShaderMask(
                                ...
                                child: Text("78 BPM", style: TextStyle(color: TColor.white.withOpacity(0.7), fontWeight: FontWeight.w700, fontSize: 18)),
                              ),
                            ],
                          ),
                        ),
                        LineChart(
                          LineChartData(
                            showingTooltipIndicators: showingTooltipOnSpots.map((index) => ShowingTooltipIndicators([LineBarSpot(tooltipsOnBar, lineBarsData.indexOf(tooltipsOnBar), tooltipsOnBar.spots[index])])).toList(),
                            lineTouchData: LineTouchData(
                              ...
                              mouseCursorResolver: (event, response) => (response == null || response.lineBarSpots == null) ? SystemMouseCursors.basic : SystemMouseCursors.click,
                              getTouchedSpotIndicator: (barData, spotIndexes) => spotIndexes.map((index) => TouchedSpotIndicatorData(const FlLine(color: Colors.red), FlDotData(show: true, getDotPainter: (spot, percent, barData, index) => FlDotCirclePainter(radius: 3, color: Colors.white, strokeWidth: 3, strokeColor: TColor.secondaryColor1)))).toList(),
                              touchTooltipData: LineTouchTooltipData(
                                getTooltipColor: (data) => TColor.secondaryColor1,
                                getTooltipItems: (spots) => spots.map((spot) => LineTooltipItem("${spot.x.toInt()} menit yang lalu", const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold))).toList(),
                              ),
                            ),
                            ...
                            Expanded(
                              ...
                              children: [
                                ...
                                ShaderMask(
                                  blendMode: BlendMode.srcIn,
                                  shaderCallback: (bounds) => LinearGradient(colors: TColor.primaryG).createShader(Rect.fromLTRB(0, 0, bounds.width, bounds.height)),
                                  child: Text("4 Liter", style: TextStyle(color: TColor.white.withOpacity(0.7), fontWeight: FontWeight.w700, fontSize: 14)),
                                ),
                                    ...
                                    return Row(
                                      ...
                                      children: [
                                        Column(
                                          ...
                                          children: [
                                            Container(margin: const EdgeInsets.symmetric(vertical: 4), width: 10, height: 10, decoration: BoxDecoration(color: TColor.secondaryColor1.withOpacity(0.5), borderRadius: BorderRadius.circular(5))),
                                            if (!isLast) DottedDashedLine(height: media.width * 0.078, width: 0, dashColor: TColor.secondaryColor1.withOpacity(0.5), axis: Axis.vertical)
                                          ],
                                        ),
                                        const SizedBox(width: 10),
                                        Column(
                                          ...
                                          children: [
                                            ...
                                            ShaderMask(
                                              blendMode: BlendMode.srcIn,
                                              shaderCallback: (bounds) => LinearGradient(colors: TColor.secondaryG).createShader(Rect.fromLTRB(0, 0, bounds.width, bounds.height)),
                      ...
                      children: [
                        Container(
                          ...
                          child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text("Tidur", style: TextStyle(color: TColor.black, fontSize: 12, fontWeight: FontWeight.w700)),
                                ShaderMask(
                                  blendMode: BlendMode.srcIn,
                                  shaderCallback: (bounds) => LinearGradient(colors: TColor.primaryG).createShader(Rect.fromLTRB(0, 0, bounds.width, bounds.height)),
                                  child: Text("8j 20m", style: TextStyle(color: TColor.white.withOpacity(0.7), fontWeight: FontWeight.w700, fontSize: 14)),
                                ),
                                const Spacer(),
                        ...
                        Container(
                          ...
                          child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text("Kalori", style: TextStyle(color: TColor.black, fontSize: 12, fontWeight: FontWeight.w700)),
                                ShaderMask(
                                  blendMode: BlendMode.srcIn,
                                  shaderCallback: (bounds) => LinearGradient(colors: TColor.primaryG).createShader(Rect.fromLTRB(0, 0, bounds.width, bounds.height)),
                                  child: Text("760 kKal", style: TextStyle(color: TColor.white.withOpacity(0.7), fontWeight: FontWeight.w700, fontSize: 14)),
                                ),
                                const Spacer(),
                                Container(
                                  alignment: Alignment.center,
                                    ...
                                    child: Stack(
                                      ...
                                      children: [
                                        Container(
                                          ...
                                          child: FittedBox(child: Text("230kKal\ntersisa", textAlign: TextAlign.center, style: TextStyle(color: TColor.white, fontSize: 11))),
                                        ),
                                        SimpleCircularProgressBar(progressStrokeWidth: 10, backStrokeWidth: 10, progressColors: TColor.primaryG, backColor: Colors.grey.shade100, valueNotifier: ValueNotifier(50), startAngle: -180),
                                      ],
                                    ),
                                  ),
                                )
                    ...
                    Container(
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton(
                            ...
                            hint: Text("Weekly", textAlign: TextAlign.center, style: TextStyle(color: TColor.white, fontSize: 12)),
                          ),
                        )),
                  ],
                ),
                    ...
                    child: LineChart(
                      LineChartData(
                        ...
                        lineTouchData: LineTouchData(
                          ...
                          mouseCursorResolver: (event, response) => (response == null || response.lineBarSpots == null) ? SystemMouseCursors.basic : SystemMouseCursors.click,
                          getTouchedSpotIndicator: (barData, spotIndexes) => spotIndexes.map((index) => TouchedSpotIndicatorData(const FlLine(color: Colors.transparent), FlDotData(show: true, getDotPainter: (spot, percent, barData, index) => FlDotCirclePainter(radius: 3, color: Colors.white, strokeWidth: 3, strokeColor: TColor.secondaryColor1)))).toList(),
                          touchTooltipData: LineTouchTooltipData(
                            getTooltipColor: (data) => TColor.secondaryColor1,
                            getTooltipItems: (spots) => spots.map((spot) => LineTooltipItem("${spot.x.toInt()} menit yang lalu", const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold))).toList(),
                          ),
                        ...
                        gridData: FlGridData(
                          ...
                          getDrawingHorizontalLine: (value) => FlLine(color: TColor.gray.withOpacity(0.15), strokeWidth: 2),
                        ),
                        borderData: FlBorderData(show: true, border: Border.all(color: Colors.transparent)),
                      ),
                    )),
  ...
  LineTouchData get lineTouchData1 => LineTouchData(
        ...
        touchTooltipData: LineTouchTooltipData(getTooltipColor: (data) => Colors.blueGrey.withOpacity(0.8)),
      );
  ...
  Widget bottomTitleWidgets(double value, TitleMeta meta) {
    ...
    Widget text;
    switch (value.toInt()) {
      case 1: text = Text('Mingg', style: style); break;
      case 2: text = Text('Sen', style: style); break;
      case 3: text = Text('Sel', style: style); break;
      case 4: text = Text('Rab', style: style); break;
      case 5: text = Text('Kam', style: style); break;
      case 6: text = Text('Jum', style: style); break;
      case 7: text = Text('Sab', style: style); break;
